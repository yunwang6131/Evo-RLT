...
"""Local-only entry point for the CRP PI05 executor without RTC.

All observation, transfer timing, action-chunk, and robot execution logic comes
from ``pi05_no_rtc.py``.  This entry point only removes remote service options
and fixes PI05 communication to ``127.0.0.1:8002``.
"""
...
import pi05_no_rtc as executor
...
def parse_local_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the CRP PI05 executor against the local server on port 8002.",
