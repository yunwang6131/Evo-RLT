...
"""Minimal HTTP action-chunk server for PI05.

Mock mode is self-contained and mirrors the CRP observation contract::

    uv run python examples/runing_service/pi05_http_server.py --mock --task "pick the cube"

Load a real LeRobot PI05 checkpoint by pointing at its ``pretrained_model`` dir::

    uv run python examples/runing_service/pi05_http_server.py \\
        --policy_path outputs/pi05_all_task2/checkpoints/140000/pretrained_model \\
        --task "pick the cube" \\
        --device cuda

The API matches the ACT HTTP service: ``GET /health``, ``GET /metadata`` and
``POST /predict``. Images are JPEG/base64; state-like features are JSON arrays.
PI05 additionally requires a language ``task`` (server default and/or per request).
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="HTTP action-chunk server for real or simulated PI05 inference.",
...
class PI05Service:
    def __init__(
        ...
        *,
        ...
        self.mock = mock
        self.default_task = default_task.strip()

        if mock:
            # Match the CRP / ACT wire contract used by existing executors.
            self.input_features = {
            ...
            self.enable_subtask_loss = False
            return

        path = str(policy_path.resolve())
        logger.info("Loading PI05 checkpoint from %s", path)
        self.policy = PI05Policy.from_pretrained(path, local_files_only=True)
        self.policy.config.device = str(self.device)
        logger.info("Moving PI05 weights to device=%s (this can take a while)...", self.device)
        self.policy.to(self.device)
        self.policy.eval()
        logger.info(
            "Loading PI05 pre/post processors (may download/load PaliGemma tokenizer if not cached)..."
        )
        ...
        logger.info("PI05 processors ready")
        self.input_features = {
        ...
        if len(action_features) != 1:
            raise ValueError(f"Expected one PI05 output feature, got {len(action_features)}")
        ...
        self.hierarchical_inference = bool(getattr(self.policy.config, "hierarchical_inference", False))
        self.enable_subtask_loss = bool(getattr(self.policy.config, "enable_subtask_loss", False))
        logger.info(
            "PI05 service init done: action_dim=%s chunk=%s/%s",
            self.action_dim,
        ...
        if not self.default_task:
            logger.warning(
                "PI05 server started without --task; each /predict request must include a non-empty 'task'."
            )
    ...
    def metadata(self) -> dict[str, Any]:
        return {
            ...
            "requires_task": True,
            "hierarchical_inference": self.hierarchical_inference,
            "enable_subtask_loss": self.enable_subtask_loss,
        }

    def _resolve_task(self, payload: dict[str, Any]) -> str:
        task = payload.get("task", self.default_task)
        if task is None:
        ...
        if not task:
            raise ValueError(
                "PI05 requires a non-empty language task. Pass --task on the server "
                "or include 'task' in the /predict payload."
            )
        return task
    ...
    def predict(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.lock:
            if self.mock:
                ...
                actions, predicted_subtask = self._pi05_actions(observation, requested, task)
            if self.latency_s:
        ...
        if predicted_subtask is not None:
            result["predicted_subtask"] = predicted_subtask
        return result
    ...
    def _pi05_actions(
        self, observation: dict[str, np.ndarray], requested: int, task: str
        ...
        with torch.inference_mode():
            ...
            actions = torch.stack(processed, dim=1).squeeze(0).detach().cpu().numpy()
            predicted_subtask = getattr(self.policy, "last_predicted_subtask", None)
            return actions, predicted_subtask
...
class PI05HTTPServer(ThreadingHTTPServer):
    def __init__(
        self,
        ...
        super().__init__(address, PI05RequestHandler)
...
def main() -> None:
    ...
    logger.info("PI05 HTTP server listening on http://%s:%d", args.host, args.port)
