...
"""CRP arm HTTP PI05 executor without RTC.

This variant keeps PI05 execution strictly sequential:

1. Read the latest robot observation
2. Send one HTTP /predict request (with language ``task``)
3. Receive one complete action chunk
4. Execute the chunk row-by-row in order
5. Repeat

PI05 itself does not perform execution-side RTC in this script. The server simply
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="CRP arm HTTP PI05 executor without RTC; execute returned chunks in order.",
...
def request_action_chunk(
    ...
    *,
    ...
    predicted_subtask = result.get("predicted_subtask")
    logger.info(
        "PI05_CHUNK_TRANSFER seq=%d chunk=%s upload_bytes=%d download_bytes=%d "
        "encode_ms=%.2f serialize_ms=%.2f upload_to_first_byte_ms=%.2f server_ms=%.2f "
        "transport_est_ms=%.2f download_ms=%.2f parse_ms=%.2f total_ms=%.2f task=%r predicted_subtask=%r",
        sequence_id,
        tuple(actions.shape),
        len(request_body),
        len(response_body),
...
def main() -> None:
    ...
    if not args.skip_confirm:
        print(
            ...
            "PI05 here is plain chunk execution: upload observation+task, download one chunk, execute it in order.\n"
    ...
    try:
        ...
        while args.steps == 0 or completed_steps < args.steps:
            ...
            if args.print_action_chunks:
                logger.info(
                    "ACTION_CHUNK seq=%d columns=%s raw_shape=%s predicted_subtask=%r\n%s",
                    result["sequence_id"],
                    CRP_ACTION_KEYS,
                    tuple(actions.shape),
            ...
            if first_action_ns is not None and execution_start_ns is not None:
                logger.info(
                    "PI05_CHUNK_LATENCY seq=%d rows=%d observation_capture_ms=%.2f "
    ...
    logger.info("CRP PI05 execution finished without RTC: steps=%d", completed_steps)
